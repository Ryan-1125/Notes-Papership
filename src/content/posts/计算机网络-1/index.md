...
> ###### 时延
>
> - 端到端时延 (简化版，仅考虑传输时延): $d_{\text{端到端}} = N \cdot \frac{L}{R}$
...
> ###### 总时延
>
> - 下述时延的综合: $d_{\text{nodal}} = d_{\text{proc}} + d_{\text{queue}} + d_{\text{trans}} + d_{\text{prop}}$
>
> ---
>
> ###### 细分
>
> - 节点处理时延 (nodal processing delay, $d_{\text{proc}}$ )
>
>     -   路由器检查分组首部、决定将该分组导向何处所需的时间;
>
> - 排队时延 (queuing delay, $d_{\text{queue}}$ )
>
>     -   在队列中, 分组在链路上等待传输时所经历的时延;
>
>     -   它的长短取决于正在排队等待向链路传输的分组数量;
>
> - 传输时延 (transmission delay, $d_{\text{trans}}$ )
>
>     -   对于以先到先服务的方式传播的分组传输方式, 假设其速率 $R = a~\text{Mbps}$ , 由于传输长度为 $L$ 的数据需要时间 $\frac{L}{R}$ , 该时延称为传输时延;
>
>     -   对于低速率的链路而言, 这个时延较长, 达到微秒, 毫秒级;
>
>     -   即: 分组交换器将整个分组全部发送到链路中所需要的时间;
>
> - 传播时延 (propagation delay, $d_{\text{prop}}$ )
>
>     -   比特信号在传播信号的物理媒体中传输所需要的时间;
>
>     -   由距离决定;
>
>     -   对于光纤、双绞铜线等媒体, 其速率范围为 $2\times10^8 \sim 3\times10^8 \ \text{m/s}$;
>
>
> ---
>
> ###### 端到端时延 (end-to-end delay, $d_{\text{end-end}}$)
>
> -   下式为不考虑排队时延时的情况;
> -   $d_{\text{end-end}} = N \cdot \left( d_{\text{proc}} + d_{\text{trans}} + d_{\text{prop}} \right)$ , 其中: $N-1$ 为源与目的主机之间的路由器数量;
...
>###### 说明
>
>- 当 $\text{流量强度} > 1$ 时, 队列趋向于无限增加, 平均排队时延将逐渐增至无穷大 (如果队列无限大);
>![alt text](1.5.png)
